...
void	user_init(t_info *info)
{
	...
	info->user.rot_speed = 0.1;//임의의 값으로 초기화해줌
	info->user.pos_x = info->map.player_x + 0.5;
	info->user.pos_y = info->map.player_y + 0.5;
	//처음에 시작하는 위치 옆에 벽이 있다면 너무 가까워서 벽을 통과한 것처럼 표현이 됨
	//pos값은 double형이므로 살짝 간격을 띄워서 벽과 가까이에서 시작하지 않게 함
	...
	//처음 생성된 위치에 다시 지나갈 수 있도록 위치 정보를 파싱한 다음 '0'으로 값을 바꿔줌
}
...
int	img_conv(t_info *info)
{
	...
	x = -1;
	//DDA algorithm으로 그려야 하는 벽의 높이를 구할 수 있으니 전체 창의 세로 한 줄씩 계산을 하여 fimg.addr에 담아줌
	while (++x < info->win.win_w)
	{
		info->dda.camera_x = 2 * x / (double)info->win.win_w - 1;
		//win_w와 x의 비율을 -1 ~ 1의 범위가 나올 수 있도록 함
		info->dda.raydir_x = info->user.dir_x + info->user.plane_x * info->dda.camera_x;
		//유저의 방향벡터는 플레이어의 평면과 수직임 플레이어에서 x좌표를 향해 광선을 쏘려면 plane에
		//win_w와 x의 비율인 camera_x를 곱한 걸 더해서 광선 방향을 알 수 있다.
		...
		while (info->dda.hit == 0)
		{
			...
			if (info->map.map[info->dda.map_x][info->dda.map_y] == '1' || \
				...
				info->dda.hit = 1; //side가 0이라면 y면에 부딪힘 1이라면 x면에 부딪힘
		}
		if (info->dda.side == 0)//어안효과를 방지하기 위해 실제 거리를 구함
